...
def test_run_now_calls_consolidation(scheduler):
    ...
    with patch("lumen.lumen.sleep.run_consolidation_pass") as mock_run:
        scheduler.run_now()
...
def test_exception_in_consolidation_caught_and_logged(scheduler):
    ...
    with (
        patch("lumen.lumen.sleep.run_consolidation_pass") as mock_run,
        patch("lumen.lumen.sleep.logger") as mock_logger,
    ):

...
from lumen.lumen.curiosity import curiosity_probe, curiosity_score
...
def test_probe_returns_oldest_and_least_accessed(memory_db):
    ...
    cid1 = _insert_chunk(
        memory_db, rid, "old memory", "h1",
        vm_score=0.5, access_count=0, last_access_at=100, created_at=1
    )
    cid2 = _insert_chunk(
        memory_db, rid, "older memory", "h2",
        vm_score=0.5, access_count=0, last_access_at=50, created_at=2
    )
    cid3 = _insert_chunk(
        memory_db, rid, "recent memory", "h3",
        vm_score=0.5, access_count=5, last_access_at=10000, created_at=3
    )

    result = curiosity_probe(memory_db, limit=5)
    assert cid2 in result  # oldest access
    assert cid1 in result  # second oldest
    assert cid3 in result  # least unseen in terms of access_count? Actually cid3 has access_count=5, while cid1/cid2 have 0
...
def test_probe_limits_results(memory_db):
    ...
    for i in range(10):
        _insert_chunk(
            memory_db, rid, f"chunk {i}", f"h{i}",
            vm_score=0.5, access_count=0, last_access_at=i * 10, created_at=i
        )
...
def test_probe_skips_invalid_chunks(memory_db):
    ...
    valid = _insert_chunk(
        memory_db, rid, "valid", "hv", valid_to=None, last_access_at=10
    )
    _insert_chunk(
        memory_db, rid, "invalid", "hi", valid_to=999, last_access_at=5
    )
...
def test_probe_deduplicates(memory_db):
    ...
    cid = _insert_chunk(
        memory_db, rid, "lonely", "h1",
        vm_score=0.5, access_count=0, last_access_at=None, created_at=1
    )
...
def test_probe_variance_signal(memory_db):
    ...
    cid_low = _insert_chunk(
        memory_db, rid, "low vm", "hl", vm_score=0.1,
        access_count=10, last_access_at=1000, created_at=1
    )
    cid_high = _insert_chunk(
        memory_db, rid, "high vm", "hh", vm_score=0.9,
        access_count=10, last_access_at=1000, created_at=2
    )
    # Middle value — not an outlier
    _insert_chunk(
        memory_db, rid, "mid vm", "hm", vm_score=0.5,
        access_count=10, last_access_at=1000, created_at=3
    )
...
def test_score_invalid_chunk(memory_db):
    ...
    invalid = _insert_chunk(
        memory_db, rid, "old", "h1", valid_to=999
    )
...
def test_score_decreases_with_access(memory_db):
    ...
    cid_unseen = _insert_chunk(
        memory_db, rid, "unseen", "h1",
        vm_score=0.5, access_count=0, last_access_at=1000, created_at=1
    )
    cid_popular = _insert_chunk(
        memory_db, rid, "popular", "h2",
        vm_score=0.5, access_count=100, last_access_at=1000, created_at=2
    )
...
def test_score_older_access_is_higher(memory_db):
    ...
    cid_old = _insert_chunk(
        memory_db, rid, "old", "h1",
        vm_score=0.5, access_count=0, last_access_at=now - 2000, created_at=1
    )
    cid_recent = _insert_chunk(
        memory_db, rid, "recent", "h2",
        vm_score=0.5, access_count=0, last_access_at=now - 1000, created_at=2
    )

...
from lumen.data.schema import ensure_schema, init_db
...
from lumen.force.mnemonic.value_model import compute_vm, extract_factors
...
def test_schema_creates_all_tables(memory_db):
    ...
    expected = {"room", "locus", "chunk", "provenance", "feedback_log", "user_profile", "event_buffer_meta"}
...
def test_store_memory_inserts_chunk_fts_vector(memory_db, test_config):
    ...
    chunk_id = store_memory(
        memory_db, "hello world", room_name="test_room",
        locus_name="test_locus", embedding=emb, config=test_config
    )
    assert chunk_id > 0
...
def test_retrieve_returns_results_ranked(memory_db, test_config):
    ...
    for i in range(5):
        ...
        store_memory(
            memory_db, f"memory number {i}", room_name="retrieve_room",
            locus_name="locus_a", embedding=emb, config=test_config
        )
...
def test_interference_weakens_old_chunk(memory_db, test_config):
    ...
    cid1 = store_memory(
        memory_db, "interference test one", room_name="interf_room",
        locus_name="same_locus", embedding=emb, config=test_config
    )
    vm_before = memory_db.execute("SELECT vm_score FROM chunk WHERE chunk_id = ?", (cid1,)).fetchone()[0]
    # Nearly identical embedding to trigger interference
    emb2 = emb * 0.99
    emb2 = emb2 / np.linalg.norm(emb2)
    store_memory(
        memory_db, "interference test two", room_name="interf_room",
        locus_name="same_locus", embedding=emb2, config=test_config
    )
    vm_after = memory_db.execute("SELECT vm_score FROM chunk WHERE chunk_id = ?", (cid1,)).fetchone()[0]
...
def test_decay_reduces_vm_score(memory_db):
    ...
    released = ebbinghaus_decay(memory_db, user_half_life_days=1.0)
...
def test_context_assembly_respects_budget(memory_db, test_config):
    ...
    chunks = [
        RetrievedChunk(
            chunk_id=i, room_name="r", locus_name="l", content="x" * 20,
            provenance_id=None, rrf_score=1.0, vm_score=0.8,
            frqad_score=0.9, recency_hours=1.0, final_score=1.0 - i * 0.01
        )
        for i in range(20)
    ]
    tfc = TFCState()
...
def test_compute_vm_factors():
    vm, factors = compute_vm("I need to schedule a meeting", {}, "user_input",

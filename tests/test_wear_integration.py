...
class TestWearAwareBatcher:
    def test_flush_sync_commits_batch(self, memory_db):
        ...
        batcher.queue.append(
            ("INSERT INTO room(name, room_type) VALUES (?, ?)", ("alpha", "domain"))
        )
        batcher.queue.append(
            ("INSERT INTO room(name, room_type) VALUES (?, ?)", ("beta", "domain"))
        )
...
class TestEbbinghausDecayBatcher:
    def test_stages_updates_without_immediate_flush(self, memory_db):
        ...
        room_id = memory_db.execute(
            "SELECT room_id FROM room WHERE name = ?", ("room1",)
        ).fetchone()[0]
        memory_db.execute("INSERT INTO locus(room_id, name) VALUES (?, ?)", (room_id, "l1"))
        locus_id = memory_db.execute(
            "SELECT locus_id FROM locus WHERE name = ?", ("l1",)
        ).fetchone()[0]
        for i in range(5):
            memory_db.execute(
        ...
        row = memory_db.execute(
            "SELECT vm_score FROM chunk WHERE content_hash = ?", ("hash0",)
        ).fetchone()
...
class TestBudgetEvictionBatcher:
    def test_stages_resolution_updates(self, memory_db):
        ...
        # Before flush
        row = memory_db.execute(
            "SELECT resolution, optical_level FROM chunk WHERE chunk_id = 1"
        ).fetchone()
...
class TestConsolidationBatcher:
    def test_stages_dedup_updates(self, memory_db, test_config):
        ...
        losers = memory_db.execute(
            "SELECT chunk_id FROM chunk WHERE content_hash = ? AND valid_to IS NOT NULL",
            ("samehash",),
        ).fetchall()
        assert len(losers) == 2
